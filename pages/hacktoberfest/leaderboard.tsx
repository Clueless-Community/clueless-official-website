                        ...
                        {leaderBoardDataArray.map((data, i) => {
                            return (
                                <tr className={`${i % 2 === 0 && 'bg-[#DBE0EB]'} rounded-md`} key={i}>
                                    ...
                                    <td className={`my-2 flex justify-start items-center xl:space-x-4 space-x-2 w-full`}><img src={data.avatarUrl} className='w-16 rounded-full border-dashed border-2 border-blue-400 text-sm ' alt='' /><span className='max-w-[113px] truncate sm:max-w-fit'>{data.userName}</span></td>
