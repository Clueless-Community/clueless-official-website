...
const WhatIsClueLess: React.FC = () => {
  ...
  return (
    <div className="md:grid lg:grid-cols-2 md:mx-40 mt-10 flex flex-col-reverse text-center lg:text-left rounded-tr-3xl rounded-br-3xl">
      <div className="md:bg-[#F2F2F2] lg:rounded-l-3xl md:rounded-t-3xl lg:rounded-tr-none pt-20 pb-10 relative border-dashed lg:border-t-2 lg:border-b-2 lg:border-l-2 md:border-l-2 md:border-r-2 md:border-t-2 border-black border-opacity-30">
      ...
      >
        <div className="xl:px-24 md:px-10 flex flex-col items-center w-11/12 mx-auto my-auto">
          {state === 0 && (
            ...
            >
              ...
              <p className="text-gray-200 mt-8">
                ...
                encourage development ! We provide the best resources, conducts
                contests, hackathons, workshops, sessions to test your skills, a
          ...
          {state === 1 && <OpenSource />}
          {state === 2 && <Resources />}
          {state === 3 && <WeeklyChallenges />}
          {state === 4 && <BuildProjects />}
        </div>
      </div>
    </div>
  );
