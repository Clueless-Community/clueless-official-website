...
const UpcomingEventCard: React.FC<dataProps> = ({
  ...
  return (
    <div className=" bg-[#1954ca25] border-dashed border-2 border-opacity-30 border-black font-nunito xl:flex-row flex-col flex xl:justify-between xl:p-8 p-3 rounded-xl relative space-y-3 xl:space-y-0">
      <section
        className={`flex xl:justify-start xl:space-x-8 space-x-4  pb-4 xl:pb-0 w-full`}
      >
        <div>
          <img
            src={eventposter}
            className="rounded-full h-auto w-36 object-cover xl:h-[200px] xl:w-[200px]"
            alt=""
          />
        </div>
        <div className="flex flex-col xl:justify-center  space-y-16 xl:text-xl">
          <div
            className={`flex flex-col xl:justify-center justify-between md:space-y-4`}
          >
            ...
            >
              <div
                className={`flex 2xl:flex-row flex-col 2xl:space-x-2 justify-start`}
              >
      ...
      <div className="space-y-2 md:hidden">
        <h1 className="xl:text-4xl text-xl xl:hidden font-semibold">
          {' '}
          {heading}
        </h1>
      ...
      <div className="text-right flex items-center space-x-4 h-fit">
        <Link href={`/events/[event_id]`} as={`/events/${eventId}`} passHref>
          <button className="bg-skin-main px-4 xl:py-3 py-2 rounded-md text-white font-semibold xl:text-xl text-lg">
            View
          </button>
        </Link>
        <div>
          <Link
            ...
            as={`/admin/events/${eventId}`}
            passHref
          >
            <button className="bg-skin-main p-2 rounded-full text-white font-semibold xl:text-xl text-lg">
              <EditIcon />
            </button>
          </Link>
        </div>
        ...
        <div className="space-y-4 p-6">
          <div id="alert-dialog-title">
            <img
              ...
              alt="warning"
              className="text-sky-500 text-5xl mx-auto"
            />
          </div>
          <div className="space-y-4 h-fit flex  flex-col justify-center items-center">
            ...
            <h1 className="text-lg font-nunito text-zinc-500">
              Type “Delete” in the box below to confirm your deletion.
            </h1>
            <input
              type="text"
              className="text-xl rounded-md focus:outline-none px-4 py-3 border-black border border-opacity-30 w-full"
              value={deleteInput}
              placeholder="DELETE"
          ...
          <div className="w-10/12 mx-auto text-center">
            {deleteInput === 'DELETE' ? (
              <button
                className="btn-blue bg-skin-red hover:bg-skin-red"
                autoFocus
              ...
              <button
                className="btn-blue bg-gray-400 hover:bg-gray-400 text-white cursor-not-allowed"
                autoFocus
                disabled
              >
                Delete
