...
const EditProject: React.FC<Props> = ({
  ...
  const [projectNameNew, _setProjectNameNew] =
    React.useState<string>(projectName);
  const [projectImageNew, setProjectImageNew] =
    React.useState<string>(projectImage);
  const [media, setMedia] = React.useState<File>();
  const [projectDescNew, _setProjectDescNew] =
    React.useState<string>(projectDesc);
  const [publicLinkNew, _setPublicLinkNew] = React.useState<string>(
    publicLink as string
  );
  const [gitHubLinkNew, _setGitHubLinkNew] = React.useState<string>(
    gitHubLink as string
  );
  const [projectTechStacks, setProjectTechStacks] =
    React.useState<{ name: string }[]>(techStacks);
  ...
  const handleImageUpload = async () => {
    ...
    const projectImageRef = ref(
      storage,
      `${uid}/project/${date.getTime()}_project.jpg`
    );
    ...
    <>
      ...
      >
        <Box sx={style} key={projectId}>
          ...
          >
            ...
            >
              <img
                src={projectImageNew}
                alt=""
                className=" rounded-lg md:w-80 h-52 absolute -z-40"
              />
          ...
          <button
            className="btn-red mt-5 float-right px-6 py-2 shadow-red-600 mr-5"
            onClick={handleClose}
          >
            Discard
          </button>
