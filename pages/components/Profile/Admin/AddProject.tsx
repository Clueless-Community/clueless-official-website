...
const AddProject: React.FC<Props> = ({ handleProjectFetch }) => {
    ...
    const [projectTechStacks, setProjectTechStacks] = React.useState<
        { name: string }[]
    >([]);
    ...
    const handleImageUpload = async () => {
        ...
        const uploadTask =
            media && uploadBytesResumable(projectImageRef, media);
        uploadTask &&
            uploadTask.on(
                "state_changed",
                (snapshot: {
    ...
    return (
        <>
            <div className="my-10" onClick={handleOpen}>
                <div className="p-6 border-2 border-dashed border-black border-opacity-60 rounded-lg h-48 bg-gray-100 flex flex-col justify-center items-center cursor-pointer hover:bg-gray-200 transition-all opacity-80 w-full dark:bg-zinc-800 dark:border-white">
                    <AddCircleIcon className="text-blue-500 hover:text-blue-600 w-16 h-16" />
                    <div className="text-center text-xl font-bold mt-2 dark:text-white">
                        Add Projects
                    </div>
                </div>
            </div>
                            ...
                            {projectImageNew ? (
                                <>
                                    <img
                                        src={projectImageNew}
                                        alt=""
                                        className=" rounded-lg md:w-80 h-52 absolute -z-40 object-cover"
                                    />
