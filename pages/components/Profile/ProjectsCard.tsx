...
const ProjectsCard: React.FC<Props> = ({
  ...
  return (
    <div className="border-opacity-60 rounded-xl shadow-lg border-[0.5px]">
      <div
      ...
      <div className="p-5 h-[21rem] flex flex-col">
        <a href={publicLink} target="_blank" rel="noreferrer" className="flex flex-row items-center flex-wrap w-[100%]">
          <h1
            title={projectName}
            className="flex-auto text-2xl mr-2 font-semibold hover:underline underline-offset-2 transition-all line-clamp-2"
          >
            {projectName}
          </h1>
          {customTitleButtons?.length ? customTitleButtons : <LinkIcon className=" cursor-pointer" />}
        </a>
        <div className="flex gap-2 flex-row my-2">
          {techStacks?.map((teckStack: { name: string }, i) => {
            if (i >= 3) return <></>;
            return <StyledChip text={teckStack.name} size="xs" rounded="2xl" key={i} />;
          })}
        </div>
        <p
          title={projectDesc}
          className="my-2 font-semibold opacity-80 flex-auto w-[100%] text-ellipsis overflow-hidden line-clamp-3"
        >
          {projectDesc}
        </p>
        <a
        ...
        >
          <div className="w-full text-center">View Repo</div>
        </a>
      </div>
    </div>
  );
